...
// TAD2012
$(document).ready(function(){if(!window.Modernizr.touch){var i=$("#navigation").offset().top;$(window).scroll(function(){$(window).scrollTop()>i?$("#navigation").addClass("fixed"):$("#navigation").removeClass("fixed")})}if(window.location.hash&&0<$(window.location.hash).length){var j=$(window.location.hash).offset().top-60;bodyelem=$.browser.safari?$("body"):$("html,body");bodyelem.scrollTop(j)}var a,c,e,g=function(){clearTimeout(e);e=setTimeout(function(){a=$("#cites li.active").next();0===a.length&&
(a=$("#cites li:first"));clearTimeout(e);$("#cites li").removeClass("active");a.addClass("active").removeClass("next");c=a.next();0===c.length&&(c=$("#cites li:first"));c.addClass("next");g()},4500)};g();var b,d,f,h=function(){clearTimeout(f);f=setTimeout(function(){b=$("#sponsor-groups li.active").next();0===b.length&&(b=$("#sponsor-groups li:first"));clearTimeout(f);$("#sponsor-groups li").removeClass("active");b.addClass("active").removeClass("next");d=b.next();0===d.length&&(d=$("#sponsor-groups li:first"));
d.addClass("next");h()},4E3)};h();0<$("#tweets").length&&getTwitters("tweets",{id:"turkuagileday",count:5,enableLinks:!0,ignoreReplies:!0,clearContents:!0,template:'%text% <a class="posted-on" href="http://twitter.com/%user_screen_name%/statuses/%id_str%/">%time%</a>'})});